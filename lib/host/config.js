...
import { DEFAULT_BROWSER_SETTINGS } from "./browser/remote-login.js";
    ...
    ctx.inject(["settings"], (sctx) => {
        if (typeof sctx.settings.register === "function") {
            ...
            current = () => {
                ...
                return { ...DEFAULT_SETTINGS, ...Object.fromEntries(Object.entries(value).filter(([, field]) => field !== undefined)) };
            };
            scope = { update: async (patch) => {
                    const id = ctx.fiber?.entry?.options.id;
                    if (id === undefined)
                        throw new Error("dsh-web-tools settings require a Loader profile entry");
                    const row = sctx.settings.describe().find(row => row.ns === id);
                    if (row === undefined)
                        throw new Error("dsh-web-tools profile settings form is unavailable");
