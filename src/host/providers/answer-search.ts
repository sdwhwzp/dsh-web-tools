...
import { unsupportedFetch } from "./free-search.ts";
import type { AnswerSearchOptions } from "../../shared/provider-options.ts";
...
export const PerplexityProvider: ProviderAdapter = {
  ...
  async search(query, maxResults, key, _base, context) {
    if (!key) throw providerError("config", "Perplexity requires an API key");
    const { signal, options } = resolveContext<AnswerSearchOptions>(context);
    ...
    if (!Array.isArray(data.citations)) throw providerError("invalid-response", "Perplexity returned no citation list");
    const sources = data.search_results === undefined
      ? data.citations.map((url): Source => ({ url: string(url) }))
...
export const DeepSeekOfficialProvider: ProviderAdapter = {
  name: "deepseek-official", label: "DeepSeek", description: "DeepSeek hosted web search", credSuffix: "DEEPSEEK",
  authentication: "required", fetchCapable: false, needsBaseUrl: false,
  async search(query, maxResults, key, _base, context) {
    if (!key) throw providerError("config", "DeepSeek search requires an API key");
    const { signal, options } = resolveContext<AnswerSearchOptions>(context);
    ...
    for (const block of blocks) {
      ...
      for (const item of records(block.citations)) citations.set(string(item.url), string(item.cited_text));
    }
    const sources: Source[] = [];
    for (const block of blocks.filter((b) => b.type === "web_search_tool_result")) {
      if (!Array.isArray(block.content)) throw providerError("server", "DeepSeek web search tool failed");
