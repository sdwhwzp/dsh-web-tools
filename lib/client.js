	...
	factory: (require) => {
		...
		*/
		...
		const api = {
			...
			remoteLoginFrame: (platform, id) => call("remote-login/frame", {
				platform,
				id
			}),
			remoteLoginInput: (platform, id, input) => call("remote-login/input", {
				platform,
				id,
				input
			}),
			remoteLoginClose: (platform, id) => call("remote-login/close", {
				platform,
				id
			})
		};
		...
		function remoteLoginStatusKey(state) {
		...
		*/
		...
		//#endregion
		//#region src/client/RemoteLoginModal.tsx
		...
		function RemoteLoginModal({ platform, t, onClose }) {
			...
			(0, react.useEffect)(() => {
				...
				opening.current.then(async (created) => {
					owned = created;
					if (cancelled) {
						if (!active.current) await api.remoteLoginClose(platform, created.id);
						return;
					}
				...
				return () => {
					...
					queueMicrotask(() => {
						if (owned && !active.current) api.remoteLoginClose(platform, owned.id).catch(() => {});
					});
				};
			}, [platform]);
			const send = (input) => {
			...
			const point = (event) => {
				...
				return {
					x: Math.max(0, Math.min((view?.width ?? 1) - 1, (event.clientX - rect.left) * (view?.width ?? 1) / rect.width)),
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				open: true,
				onClose,
				title: `${t("remoteLoginTitle")} · ${t(platform === "x" ? "xTitle" : "xiaohongshuTitle")}`,
				headless: true,
				className: "dswt-remote-login-dialog",
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("strong", { children: [
								t("remoteLoginTitle"),
								" · ",
								t(platform === "x" ? "xTitle" : "xiaohongshuTitle")
							] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
								type: "button",
								variant: "ghost",
								size: "sm",
								onClick: onClose,
								children: t("remoteLoginClose")
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							...
							children: t("remoteLoginHint")
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							role: "status",
							style: { fontSize: 13 },
							children: t(remoteLoginStatusKey(view?.state))
						}),
						(error || view?.error) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							role: "alert",
							style: {
								color: state.danger,
								fontSize: 13
							},
							children: error || view?.error
						}),
						view?.image && pending && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("img", {
							src: view.image,
							alt: t("remoteLoginScreen"),
							tabIndex: 0,
							draggable: false,
							onKeyDown: key,
							onPointerDown: (e) => pointer(e, "down"),
							onPointerUp: (e) => pointer(e, "up"),
							onPointerCancel: (e) => pointer(e, "up"),
							onPointerMove: (event) => {
								if (dragging.current) {
									nextMove.current = {
										type: "pointer",
							...
							onWheel: (event) => {
								...
								send({
									type: "wheel",
									...point(event),
									deltaY: Math.max(-2e3, Math.min(2e3, event.deltaY))
								}).catch(() => {});
							},
							style: {
								width: "100%",
						...
						pending && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("form", {
							onSubmit: (event) => {
								event.preventDefault();
								sendDraft();
							},
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
									...
									"aria-label": t("remoteLoginText"),
									placeholder: t("remoteLoginText"),
									style: {
										flex: "1 1 240px",
										padding: "8px 10px",
										color: text.primary,
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
									...
									children: t("remoteLoginSendText")
								}),
								[
									"Backspace",
									"Tab",
									"Enter"
								].map((name) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
									type: "button",
									variant: "outline",
									onClick: () => void send({
										type: "key",
									...
									children: t(`remoteLoginKey${name}`)
								}, name))
							]
						})
					]
...
<path d="M81.1293 91.3261H192.438C192.729 92.501 193.019 93.6939 193.291 94.8869H267.128C264.479 86.6084 261.067 78.6734 256.948 71.1722H186.051C186.504 72.3471 186.94 73.5401 187.375 74.733H86.1921C85.6296 76.3056 85.0852 77.8781 84.5771 79.4687H12.464C11.3934 81.8004 10.3772 84.1683 9.43359 86.5904H82.4177C81.9822 88.1629 81.5467 89.7354 81.1474 91.3261H81.1293Z" fill="#1D1C1A"/>
		...
		function RefreshButton(props) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconRefreshOutlineRegular, { size: 13 })
				})
			});
		}
		...
		*/
		...
		function WebToolsSection(props) {
			...
			const [remoteLoginPlatform, setRemoteLoginPlatform] = (0, react.useState)(null);
      ...
      ` }),
					remoteLoginPlatform && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(RemoteLoginModal, {
						platform: remoteLoginPlatform,
						t,
						onClose: () => {
							setRemoteLoginPlatform(null);
							loadPlatformStatus();
						}
					}, remoteLoginPlatform),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("section", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SettingsGroup, { children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SettingsRow, {
						...
						trailing: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
							...
							icon: !editingOrder ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconEditOutlineRegular, { size: 13 }) : void 0,
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("section", { children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(SettingsGroup, {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(SettingsRow, {
								...
								title: t("xTitle"),
								subtitle: (config.platformEnabled?.x ?? true) === false ? t("platformDisabled") : platformState?.platforms?.x?.lastError ?? (platformLoginMessage(platformState?.platforms?.x) ? t(platformLoginMessage(platformState?.platforms?.x)) : platformState?.platforms?.x?.authenticated ? `${t("platformAccountPrefix")}${platformState.platforms.x.account?.handle ?? t("platformConnected")}` : platformState?.platforms?.x?.sessionEstablished ? t("platformVerifying") : t("platformNotLoggedIn")),
								trailing: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									style: {
										display: "inline-flex",
										alignItems: "center",
...
`;
		...
		*/
		...
		const zhDict = {
			...
			xTitle: "Twitter / X",
			loginButton: "登录",
			remoteLoginTitle: "网页远程登录",
			remoteLoginClose: "关闭登录窗口",
			remoteLoginHint: "直接点击下方页面登录，可扫码、输入验证码或拖动滑块。输入中文或粘贴内容时，先点击页面中的输入框，再使用下方文字输入栏。登录来源由所有账号共享。",
			remoteLoginStarting: "正在打开服务端登录页面…",
			remoteLoginReady: "请在下方页面完成登录",
			remoteLoginSuccess: "登录成功，已保存共享登录状态。可以关闭此窗口。",
			remoteLoginFailed: "登录未完成，请关闭后重试",
			remoteLoginScreen: "远程登录页面，可点击、输入和拖动",
			remoteLoginText: "文字、手机号、验证码或密码（发送后清空）",
			remoteLoginSendText: "输入到页面",
			remoteLoginKeyBackspace: "退格",
			remoteLoginKeyTab: "下一输入框",
			remoteLoginKeyEnter: "回车",
			platformLoginPending: "等待在服务端浏览器中完成登录…",
			platformBrowserMissing: "服务端未安装受支持的浏览器，无法登录",
			platformDisplayMissing: "服务端没有可用的图形桌面，无法打开登录窗口",
			platformLoginHostHint: "点击登录可在网页中操作服务端浏览器。登录状态保存在服务器，供所有账号共享。",
		...
		const enDict = {
			...
			xTitle: "Twitter / X",
			loginButton: "Login",
			remoteLoginTitle: "Remote login",
			remoteLoginClose: "Close login window",
			remoteLoginHint: "Use the page below to sign in, scan a QR code, enter a verification code or drag a slider. For pasted text or IME input, select the page field first, then use the text field below. Signed-in sources are shared by all accounts.",
			remoteLoginStarting: "Opening the Host login page…",
			remoteLoginReady: "Complete login on the page below",
			remoteLoginSuccess: "Signed in. The shared session is saved; you can close this window.",
			remoteLoginFailed: "Login did not complete. Close this window and try again.",
			remoteLoginScreen: "Remote login page with pointer, keyboard and drag controls",
			remoteLoginText: "Text, phone, verification code or password (cleared after sending)",
			remoteLoginSendText: "Type into page",
			remoteLoginKeyBackspace: "Backspace",
			remoteLoginKeyTab: "Next field",
			remoteLoginKeyEnter: "Enter",
			...
			platformLoginHostHint: "Login opens an interactive Host browser view in this page. The signed-in profile stays on the server and is shared by all accounts.",
