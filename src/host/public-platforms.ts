...
import { PUBLIC_PLATFORMS, type PublicPlatform } from "../shared/search-policy.ts";
import { normalizeSources, plainText, record, records, searchJson, string } from "./providers/search-response.ts";
import { providerError, type SearchOutcome, type Source } from "./providers/types.ts";

const PREFIXES: Record<string, PublicPlatform> = {
...
export function publicPlatformQuery(query: string): { platform: PublicPlatform; query: string } | undefined {
  const match = query.trim().match(/^([^:：]+)[:：]\s*([\s\S]*)$/);
  const platform = match && PREFIXES[match[1].toLowerCase()];
  if (!platform) return undefined;
  if (!match[2].trim()) throw providerError("bad-request", "A platform prefix must be followed by a search query");
  return { platform, query: match[2].trim() };
}

/** Search a public platform; V2EX only filters the current hot-topic feed. */
export async function searchPublicPlatform(platform: PublicPlatform, query: string, limit: number, signal?: AbortSignal, language = "zh"): Promise<SearchOutcome> {
  const headers = { "user-agent": "dsh-web-tools (https://github.com/sdwhwzp/dsh-web-tools)", accept: "application/json" };
  const get = (url: string, extra: Record<string, string> = {}) => searchJson(url, { headers: { ...headers, ...extra }, signal });
  ...
  switch (platform) {
    ...
    case "bilibili": {
      ...
      if (data.code !== 0) throw providerError("server", `Bilibili: ${string(data.message) || "search failed"}`);
    ...
    case "stackoverflow": {
      ...
      if (data.error_id) throw providerError("server", `Stack Overflow: ${string(data.error_message)}`);
