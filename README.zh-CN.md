...
搜索引导、强制联网快照及纠正通知使用消息来源 `plugin:dsh-web-tools`，与已迁移历史及会话 V4 格式一致。
...
小红书站内搜索会等待可见且可输入的首页搜索框，兼容传统顶部输入框与新版首页文本框；新版通过回车提交。搜索框不可用时返回 `search-control-unavailable`，搜索结果页未就绪时返回 `navigation-failed`。
