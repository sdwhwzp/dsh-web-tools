...
import { providerError, throwIfHttp, type Source } from "./types.ts";
...
export function record(value: unknown): SearchRecord {
...
export function records(value: unknown): SearchRecord[] {
  if (!Array.isArray(value)) throw providerError("invalid-response", "Search service returned an invalid result list");
...
export function plainText(value: unknown): string {
  ...
  for (const el of Array.from(document.querySelectorAll("script,style"))) el.remove();
  return (document.body.textContent ?? "").replace(/\s+/g, " ").trim();
}

/** Deduplicate HTTP(S) sources, rejecting executable URLs from remote results. */
export function normalizeSources(sources: Source[], limit: number): Source[] {
  const seen = new Set<string>();
  return sources.filter((source) => {
    let url: URL;
    try { url = new URL(source.url); } catch { return false; }
    if (!['http:', 'https:'].includes(url.protocol) || url.username || url.password || seen.has(url.href)) return false;
    seen.add(url.href);
    source.url = url.href;
    return true;
  }).slice(0, limit);
...
export async function searchText(response: Response): Promise<string> {
  const reader = response.body?.getReader();
  if (!reader) throw providerError("invalid-response", "Search service returned an empty response");
...
export async function searchJson(url: string, init: RequestInit = {}): Promise<unknown> {
  init.signal?.throwIfAborted();
  const response = await fetchWithProxy(url, { ...init, redirect: "error" });
  throwIfHttp(new URL(url).hostname, response);
  try { return JSON.parse(await searchText(response)); }
  catch (error) {
    if (error instanceof SyntaxError) throw providerError("invalid-response", "Search service returned invalid JSON");
    throw error;
  }
}

/** Call a public MCP search tool, accepting JSON or a matching SSE JSON-RPC response. */
export async function searchMcp(url: string, tool: string, args: SearchRecord, signal?: AbortSignal): Promise<SearchRecord> {
  ...
  try {
    messages = response.headers.get("content-type")?.includes("text/event-stream")
      ...
      : [JSON.parse(text)];
  } catch { throw providerError("invalid-response", "MCP search returned invalid JSON"); }
  const message = messages.map(record).find((item) => item.id === 1);
  if (!message) throw providerError("invalid-response", "MCP search response is missing the request id");
  if (message.error) throw providerError("server", `MCP search failed: ${string(record(message.error).message)}`);
  const result = record(message.result);
  if (result.isError) throw providerError("server", "MCP search tool reported an error");
  return result;
}

/** Extract text blocks from an MCP tool result. */
export function mcpText(result: SearchRecord): string {
