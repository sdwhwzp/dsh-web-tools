...
export function WebToolsSection(props) {
    ...
    const [loginStarting, setLoginStarting] = useState({});
                                ...
                                : loginErrors.x ?? platformState?.platforms?.x?.lastError ?? (platformLoginMessage(platformState?.platforms?.x, loginStarting.x)
                                    ...
                                    : platformState?.platforms?.x?.authenticated
                                        ...
                                        : platformState?.platforms?.x?.sessionEstablished
                                            ? t("platformVerifying")
                                            : t("platformNotLoggedIn")), trailing: _jsxs("div", { style: { display: "inline-flex", alignItems: "center", gap: 10 }, children: [(config.platformEnabled?.x ?? true) && (platformState?.platforms?.x?.authenticated ? (_jsxs(_Fragment, { children: [_jsx(StateDot, { state: "done", size: 6 }), _jsx(Button, { size: "sm", variant: "ghost", onClick: () => void resetPlatformSession("x"), children: t("clearSessionButton") })] })) : platformState?.platforms?.x?.sessionEstablished ? (_jsx(StateDot, { state: "ongoing", size: 6 })) : (_jsx(Button, { size: "sm", variant: "outline", disabled: loginStarting.x || platformState?.platforms?.x?.loginPending, onClick: () => void loginPlatform("x"), children: t("loginButton") }))), _jsx(Switch, { checked: config.platformEnabled?.x ?? true, onChange: (v) => togglePlatform("x", v), label: t("xTitle") })] }), isLast: true })] }) }), _jsx("section", { children: _jsx(SettingsGroup, { title: t("providersLabel"), dividers: "inset", children: renderedProviders.map((p, idx) => {
