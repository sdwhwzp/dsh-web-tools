...
export function SearchModeButton({ sessionId, label = "联网搜索", unavailableLabel = "没有可用的搜索源", autoTooltip = "自动联网：Agent 会在需要时使用联网搜索", requiredTooltip = "已要求联网：回答前必须完成一次联网搜索", }) {
    ...
    const refresh = () => {
        ...
        if (pendingRef.current)
            return; // don't clobber an optimistic toggle
    ...
    const tooltip = !available
        ...
        : required
            ? requiredTooltip
            : autoTooltip;
    return (_jsxs("button", { type: "button", className: searchModeCss.trigger, "data-active": required || undefined, "data-loading": loading || undefined, "data-pending": pending || undefined, "data-unavailable": !available || undefined, "aria-busy": loading || pending || undefined, "aria-pressed": required, "aria-label": label, title: tooltip, disabled: !available || loading || pending, onMouseDown: (event) => {
            // Keep the textarea caret: toggling a mode must not steal compose focus.
            event.preventDefault();
        }, onClick: () => {
            void toggle();
        }, children: [_jsx("span", { className: searchModeCss.icon, "aria-hidden": true, children: _jsx(IconGlobeOutlineRegular, { size: 14 }) }), _jsx("span", { className: searchModeCss.label, children: label })] }));
}
