...
import { providerError, resolveContext, throwIfHttp, type ProviderAdapter, type Source } from "./types.ts";
import { mcpText, normalizeSources, record, records, searchJson, searchMcp, searchText, string } from "./search-response.ts";
import type { FreeSearchOptions } from "../../shared/provider-options.ts";
...
export async function unsupportedFetch(): Promise<never> {
...
function resultUrl(href: string, base: string): string {
  let url: URL;
...
export function parseHtmlSearch(engine: keyof typeof HTML_ENDPOINTS, html: string, limit: number): Source[] {
  ...
  const sources = links.map((link, index): Source => {
  ...
  if (!normalized.length) throw providerError("invalid-response", `${engine} returned no usable search results`);
  return normalized;
}

function htmlProvider(name: keyof typeof HTML_ENDPOINTS, label: string): ProviderAdapter {
  return {
    ...
    async search(query, maxResults, _key, _base, context) {
      const { signal, options, hints } = resolveContext<FreeSearchOptions>(context);
      signal?.throwIfAborted();
      const params = new URLSearchParams({ q: query });
      if (name === "bing") {
        params.set("mkt", options?.market ?? "zh-CN");
        if (options?.safeSearch) params.set("adlt", options.safeSearch);
      } else {
        ...
        if (preset) params.set("df", { day: "d", week: "w", month: "m", year: "y" }[preset]);
      }
      ...
      if (response.status === 202) throw providerError("rate-limit", `${label} requires browser verification`);
...
export const AnysearchProvider: ProviderAdapter = {
  ...
  async search(query, maxResults, _key, _base, context) {
    ...
    if (data.code !== 0) throw providerError("server", `AnySearch: ${string(data.message) || "search failed"}`);
...
export function parseMcpSources(text: string, limit: number): Source[] {
...
export async function searchExaAnonymous(query: string, limit: number, signal?: AbortSignal) {
...
export function filterSourceDomains(sources: Source[], hints?: Readonly<SearchHints>): Source[] {
  const matches = (host: string, domain: string) => host === domain.toLowerCase() || host.endsWith(`.${domain.toLowerCase()}`);
