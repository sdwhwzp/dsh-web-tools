 ...
 */
export function loginUnavailableReason(
  browserAvailable: boolean,
  platform: string = process.platform,
  env: Record<string, string | undefined> = process.env,
): LoginUnavailableReason | undefined {
  if (!browserAvailable) return "browser-missing";
  if (platform === "linux" && !env.DISPLAY && !env.WAYLAND_DISPLAY) return "display-missing";
  return undefined;
}
