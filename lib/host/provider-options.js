    ...
    switch (provider) {
        case "searxng": {
            if (raw.instances !== undefined) {
                if (!Array.isArray(raw.instances) || raw.instances.length > 10)
                    throw new Error("Invalid SearXNG instance list");
                out.instances = [...new Set(raw.instances.map((value) => {
                        if (typeof value !== "string")
                            throw new Error("Invalid SearXNG instance URL");
            ...
            if (raw.instanceTimeoutMs !== undefined) {
                if (typeof raw.instanceTimeoutMs !== "number" || !Number.isInteger(raw.instanceTimeoutMs) || raw.instanceTimeoutMs < 1000 || raw.instanceTimeoutMs > 60000)
                    throw new Error("Invalid SearXNG instance timeout");
        ...
        case "ddg":
        case "ddg-lite": {
            for (const field of ["market", "region"]) {
                if (raw[field] !== undefined) {
                    if (typeof raw[field] !== "string" || !/^[a-z]{2}-[a-z]{2}$/i.test(raw[field]))
                        throw new Error(`Invalid ${field}`);
                    out[field] = raw[field];
                }
            }
            if (raw.safeSearch !== undefined) {
                if (typeof raw.safeSearch !== "string" || !["off", "moderate", "strict"].includes(raw.safeSearch))
                    throw new Error("Invalid safeSearch");
        ...
        case "deepseek-official": {
            if (raw.model !== undefined) {
                if (typeof raw.model !== "string" || !raw.model.trim() || raw.model.length > 100)
                    throw new Error("Invalid search model");
                out.model = raw.model.trim();
            }
            if (raw.maxTokens !== undefined) {
                if (typeof raw.maxTokens !== "number" || !Number.isInteger(raw.maxTokens) || raw.maxTokens < 128 || raw.maxTokens > 16384)
                    throw new Error("Invalid search token budget");
